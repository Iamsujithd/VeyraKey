...
- **Last updated:** 2026-07-28T14:10:00Z — Extension 0.6.10 removes duplicate inline account choices and reduces the field-anchored picker.
...
- **Artifact evidence:** Version 0.6.10 Chrome ZIP SHA-256 `7d20410990c79105e77525983c4fd1d0dcc990fd5796e318dbf9404e19c0a59c`; Firefox ZIP SHA-256 `7d6264c2ad91b67d25113b2f67b49e801bcc839d71ec5ea5c1f44d83f6e883ce`; source ZIP SHA-256 `559a63785f8bfb25117ee19311d3631401eefd7d080bbee4ee15391e2085e74a`; lockfile SHA-256 `eb159431d99f74d8498e15bc0b6d527ee14cfa81f261357954f0474affdf8294`.
- **Latest validation:** `CI=true pnpm check` passes with 36 test files and 191 tests, all production targets, and artifact verification. Chrome confirms that the installed unpacked extension is version 0.6.10 and is loaded from `apps/extension/.output/chrome-mv3`; the primary third-party login fixture did not render after refresh, so a new live visual interaction result is not claimed for that page. The focused regression suite and packaged script prove that version 0.6.10 applies one named-account preference rule to both locked metadata and unlocked vault responses, suppressing a blank legacy `Saved login` whenever a named exact-origin account exists. The inline picker no longer creates a second `Sign in as …` action for a single credential; one account now produces one fill-only row. Its width is reduced from 300 px to 264 px, row height from 46 px to 38 px, and authentication copy is shortened. The isolated packaged-extension runtime smoke from version 0.6.9 continues to cover extension loading, HTTPS injection, idempotent repeated delivery, dynamic/replaced login detection, service-worker restart persistence, prompt restoration, and extension-origin PRF capability without reported runtime errors. Version 0.6.9 distinguishes successful biometric verification from credential availability: when the unlocked vault has no login for the page's exact HTTPS origin, the protected surface ends in a terminal “No saved login” state, offers no meaningless biometric retry, releases no password, and explains that credentials are never reused across origins. Content-script runtime-ID access and message-listener registration also abort cleanly if Chrome replaces the extension during startup. Version 0.6.8 accepts an identical repeated credential delivery as success without rewriting either field, while mismatched prefilled usernames or passwords still fail closed. Version 0.6.7 maintains a validated local-only suggestion index containing only credential ID, username, and exact HTTPS origins. It is rebuilt after authenticated vault reads and saves, is never cloud-synced, and contains no passwords. Pointer-tracked radial gradients were removed from both inline and protected surfaces while static blur, edge highlights, depth, and accessibility fallbacks remain. Version 0.6.5 passes a bounded observed-username hint into the protected extension origin; after fresh authentication, a unique case-normalized username match fills automatically while zero or multiple named matches remain explicit choices. Version 0.6.4 uses Chrome 127+'s action-popover API to avoid a separate macOS window and retains the isolated compact-window fallback on older/unsupported browsers. A physical Touch ID ceremony remains manual release evidence.
