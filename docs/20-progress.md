...
- **Last updated:** 2026-07-28T12:30:00Z — Extension 0.6.3 replaces the oversized AutoFill window with a compact verification sheet that starts biometric verification immediately after an intentional credential selection.
...
- **Artifact evidence:** Version 0.6.3 Chrome ZIP SHA-256 `fea71fc749bca46ccae7077c3f645622e7a5b213c311b0f0b232eb6f544401da`; Firefox ZIP SHA-256 `71459f40176388becbc71f16c89ee8431bb1f182c5c7ecbecc78a11106644a28`; source ZIP SHA-256 `07628b3a997533118acf253a93fec0ae1fbe04de468a5d6d7e4a22bf2e24fb30`; lockfile SHA-256 `eb159431d99f74d8498e15bc0b6d527ee14cfa81f261357954f0474affdf8294`.
- **Latest validation:** `CI=true pnpm check` passes with 35 test files and 184 tests, all production targets, and artifact verification. The regression suite proves an already-unlocked manager still performs a fresh device ceremony, a canceled ceremony releases no credential, and the generated content script contains no legacy direct-plaintext selection protocol. Version 0.6.3 opens a compact 360×270 biometric sheet, starts the device ceremony automatically, removes the redundant submit checkbox and second biometric button, and preserves manual-password fallback. The credential chip still requires an intentional click; hover alone never releases a secret. Chrome retains its native popup-window frame because Manifest V3 does not expose a frameless system-popover API. Version 0.6.2 additionally catches both synchronous and asynchronous invalid-context failures after reload, removes the stale prompt, disconnects its observer, and stops further stale-script requests. The prior isolated real-Chromium lifecycle run verified extension loading, content-script readiness, dynamic username detection, replaced password-step detection, pending-save survival across service-worker termination/navigation, and extension-origin WebAuthn PRF capability; a physical Touch ID ceremony remains manual release evidence.
