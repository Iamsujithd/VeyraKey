...
- **Last updated:** 2026-07-28T13:50:00Z — Extension 0.6.9 makes an exact-origin miss terminal after authentication and guards content-script startup/registration during extension replacement.
...
- **Status:** Complete.
- **Scope delivered:** Exact HTTPS-origin and punycode-aware matching; contextual field-anchored suggestions; protected extension-origin biometric or master-password AutoFill with immediate relock; fresh verification for every secret release even when the manager session is already unlocked; credential-ID and submit-choice binding across the protected popup handoff; autofocus, pointer, page-restore, visibility, and dynamic-step detection; explicit fill-and-submit with ambiguous-form refusal; confirmed save/update capture; exact-origin TOTP fill; strict otpauth parsing and RFC vectors; capability-gated local QR import; unbiased CSPRNG password and 2048-token passphrase generation; best-effort clipboard clearing that preserves newer clipboard content; encrypted TOTP, tag, folder, and favorite fields; local search UI; and a root-derived authenticated rebuildable search index in IndexedDB version 5.
- **Validation:** The current whole-repository check passes with 35 test files and 184 tests. The suites cover lookalikes, IDNs, frames, ambiguity, form events, mandatory fresh manual/biometric exact-origin handoff, already-unlocked and canceled-biometric regressions, synchronous and asynchronous extension-reload invalidation, rejected-secret clearing, capture decisions, explicit confirmation, generator rejection sampling, RFC TOTP vectors, QR payloads, clipboard replacement races, encrypted item plaintext inspection, encrypted search/tamper handling, persistence migration, manifests, and production builds.
...
- **Artifact evidence:** Version 0.6.9 Chrome ZIP SHA-256 `82997382b22e1f0aaec815844881d8527555db9908ceab727d39274232fe129d`; Firefox ZIP SHA-256 `5fe8f237ca90d94b3967f5256de24d7c5660228130f615df8b1e3e814760c9ac`; source ZIP SHA-256 `3b2e674e96845704a617aae200431c4fa00000f9a1c036db53a29bba7717fdbb`; lockfile SHA-256 `eb159431d99f74d8498e15bc0b6d527ee14cfa81f261357954f0474affdf8294`.
- **Latest validation:** `CI=true pnpm check` passes with 36 test files and 190 tests, all production targets, and artifact verification. The isolated packaged-extension runtime smoke also passes extension loading, HTTPS injection, idempotent repeated delivery, dynamic/replaced login detection, service-worker restart persistence, prompt restoration, and extension-origin PRF capability without reported runtime errors. Version 0.6.9 distinguishes successful biometric verification from credential availability: when the unlocked vault has no login for the page's exact HTTPS origin, the protected surface ends in a terminal “No saved login” state, offers no meaningless biometric retry, releases no password, and explains that credentials are never reused across origins. The generic locked chooser now says it will check the exact site rather than promising a fill before the vault is authenticated. Content-script runtime-ID access and message-listener registration also abort cleanly if Chrome replaces the extension during startup. Version 0.6.8 accepts an identical repeated credential delivery as success without rewriting either field, while mismatched prefilled usernames or passwords still fail closed; this prevents a stale duplicate content-script listener from replacing a successful Touch ID fill acknowledgement with “the login form changed.” The regression suite delivers the same credential twice, requires both acknowledgements to succeed, and proves only the first delivery emits the two input events. It also proves an already-unlocked manager still performs a fresh device ceremony, a canceled ceremony releases no credential, and the generated content script contains no legacy direct-plaintext selection protocol. Version 0.6.7 maintains a validated local-only suggestion index containing only credential ID, username, and exact HTTPS origins. It is rebuilt after authenticated vault reads and saves, is never cloud-synced, and contains no passwords. This lets the locked inline menu show named accounts for deliberate selection before fresh biometric release. Pointer-tracked radial gradients were removed from both inline and protected surfaces while static blur, edge highlights, depth, and accessibility fallbacks remain. Version 0.6.6 filters blank-username records from exact-origin candidates whenever at least one usable named login exists; a regression with blank `Saved login` plus named `student` proves `student` fills automatically without a second picker. The protected surface is reduced to 300 px. Version 0.6.5 passes a bounded observed-username hint into the protected extension origin; after fresh authentication, a unique case-normalized username match fills automatically while zero or multiple named matches remain explicit choices. Version 0.6.4 uses Chrome 127+'s action-popover API to avoid a separate macOS window and retains the isolated compact-window fallback on older/unsupported browsers. Version 0.6.3 starts the device ceremony automatically, removes the redundant submit checkbox and second biometric button, and preserves manual-password fallback. The credential chip still requires an intentional click; hover alone never releases a secret. Version 0.6.2 additionally catches both synchronous and asynchronous invalid-context failures after reload, removes the stale prompt, disconnects its observer, and stops further stale-script requests. A physical Touch ID ceremony remains manual release evidence.
