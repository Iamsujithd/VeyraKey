...
- **Current implementation task:** Task 14 — local release artifacts complete; external gates open.
...
| 14 | Portfolio deployment/release | In review | Reproducible builds, Chrome/Firefox ZIPs, SBOM, hashes, smoke/demo/migration/rollback runbook | Hosted URLs, Google OAuth test account, real-browser matrix, store signing |
...
- **Open external gates:** No hosting project/configuration or production URL exists in the workspace; Google OAuth consent/domain/client and test account are unavailable; the real Chrome/Firefox/PRF matrix has not run; and neither extension ZIP is store-signed. Task 14 must not be marked complete until the evidence is attached.
