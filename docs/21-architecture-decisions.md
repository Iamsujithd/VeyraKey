...
- **Decision — target surfaces and compatibility:** HTTPS web may offer PRF only through runtime gates. Extensions retain empty host permissions and report PRF unsupported until a production RP permission and Task 9 session bridge exist. Multiple devices require explicit slot selection. No browser/authenticator combination is claimed compatible until the release matrix is exercised with real platform evidence.
