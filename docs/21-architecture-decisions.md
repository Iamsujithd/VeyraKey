...
- **Date:** 2026-07-28.
- **Status:** Accepted and hardened through extension version 0.10.0.
- **Decision — account and storage model:** The product does not introduce a password-reset or key-escrow control-plane account. A portable user vault is identified by its authenticated encrypted bootstrap and immutable history. New setup recommends a configured user-owned Google Drive or OneDrive provider, but always creates keys locally, requires the Recovery Kit drill before the first upload, and offers an explicit device-only mode. Connecting another provider later is a migration/synchronization action over the same encrypted revisions. A new device restores with the same provider identity plus Recovery Kit and establishes a new local master-password wrapper.
- **Decision — registration generation:** Strong-password UI is shown only for connected, writable password inputs explicitly marked `new-password` or contained in conservatively identified signup/register/create-account forms. `current-password`, password-reset, recovery, and change-password surfaces always exclude generation. The unconstrained default mirrors Apple's documented 20-character construction: three six-character alphanumeric groups separated by two hyphens, containing exactly 16 lowercase letters, one uppercase letter, and one digit. Website `passwordrules`, `minlength`, `maxlength`, pattern, and described requirements trigger a compatible site-specific construction rather than changing the default opportunistically. Apple-style alternatives let the user request a new strong password, letters-and-numbers-only output when compatible, an easy-to-type alphabet without look-alike characters, or their own password. A generated value is retained only for the current form lifetime, fills matching registration/confirmation inputs with browser-compatible events after an explicit user choice, and is offered to the normal explicit encrypted capture flow. Mismatched confirmation fields fail closed.
- **Decision — identity profiles:** Name, nickname, email, phone, organization, date of birth, age, and address data are a strict bounded `identity-profile` item. They use the same independent item key, authenticated immutable revision, encrypted persistence, encrypted local search index, sync, conflict, backup, and restore boundaries as other vault items. The locked suggestion index continues to contain login metadata only. Profile lookup requires an unlocked/resumable vault, exact top-level HTTPS origin validation, explicit profile selection, and a field-kind allowlist. Only the selected field value is released. Login email fields remain credential fields; explicit contact fields on registration forms remain eligible.
- **Decision — breach state:** Every newly saved or updated login, including extension capture, performs the existing k-anonymous HIBP range check and stores only the bounded result (`found` count, `not-found`, or `unavailable`) plus timestamp inside the encrypted item. Manual retry remains available. Network failure never blocks the encrypted save and is represented explicitly rather than as a clean result.
- **Decision — UI and packaging:** Cloud/local choice is part of setup instead of a settings-only afterthought. Settings retains disconnect, retry, and migration actions. Secondary text must meet the project’s readable contrast tokens in light and dark extension surfaces. The popup vault manager is dynamically split so every production JavaScript chunk remains below the 900 kB release ceiling.
- **Rejected:** Server-side user/key escrow, asking end users for OAuth application IDs, treating a cloud token as a decryption factor, silent credential generation/filling/saving, generation on login forms, plaintext profile indexes, releasing a whole profile when one field was selected, treating an unavailable breach check as safe, and weakening the artifact budget to accommodate feature growth.
- **Consequences and limits:** The same user can operate on multiple devices without a project-owned account database, but must retain the Recovery Kit and access to the selected provider; local-only users accept that device loss is unrecoverable unless they export or migrate. Provider OAuth, physical biometrics, and browser-store installation remain platform/manual evidence and are not implied by automated tests.

## ADR-037: Compact browser controller and progressive vault manager

- **Date:** 2026-07-29.
- **Status:** Accepted for extension version 0.10.0.
- **Context:** A browser-action popup had accumulated the complete vault manager and every item field. This obscured the page, duplicated field-level AutoFill, produced an unavoidable rectangular browser popup around a second rounded panel, and made routine credential use resemble an administration workflow.
- **Decision — surface ownership:** The browser action is a compact contextual controller. It lists only exact-origin saved account names when matches exist and otherwise stays quiet apart from the dedicated Passwords-manager action. Authentication and secret release remain in protected extension-origin flows. The browser action never embeds the full manager or fabricates a generic “check vault” action for unmatched pages.
- **Decision — manager hierarchy:** The unlocked manager defaults to the encrypted item library and search. Creating or editing an item is an explicit action that opens a focused, dismissible sheet. Saving, deleting, canceling, locking, and context loss close the editor and clear secret inputs. Tools and settings remain separate navigation destinations.
- **Decision — material and accessibility:** Glass is a sparse functional layer for transient controls, navigation, and the editor sheet; content-heavy library and settings surfaces use stable opaque backgrounds. The regular material keeps readable contrast in text-heavy surfaces, adapts to light and dark appearance, and has solid reduced-transparency, reduced-motion, and forced-colors fallbacks. Decorative pointer spotlights are excluded.
- **Decision — platform honesty:** Chrome controls the outer browser-action window and does not expose Apple's private native material or Safari AutoFill UI. CSS may remove doubled backgrounds and visually clip the document, but the product does not claim pixel-identical native Liquid Glass. It adopts the interaction hierarchy, restraint, adaptability, and contextual placement described by Apple.
- **Rejected:** Full manager in the browser action, a giant default form, glass on every content card, pointer-following light effects, auto-opening the manager on every login page, and claiming Chrome HTML can reproduce Apple's proprietary system surface.
- **Evidence:** Compact-popup and manager-mode component tests, item-editor lifecycle tests, semantic button/heading assertions, production Chrome/Firefox builds, and documented light/dark/accessibility fallbacks.
...
- **Consequences:** The reproducible local Task 14 release candidate can be accepted without public hosting, which was intentionally removed. OAuth/test-account evidence, the physical-device matrix, store signing, and independent review remain explicit public-release gates and are never implied by a local build.
...
- **Date:** 2026-07-27.
- **Status:** Accepted and hardened through extension version 0.7.0.
- **Decision:** A compact field-anchored Passwords chooser is triggered by trusted focus and pointer events and is refreshed for page restoration, visibility changes, autofocus, and dynamically replaced login steps. A local-only rebuildable suggestion index stores only credential ID, username, and canonical HTTPS origins so a locked chooser can present accounts before secret release; it is never synced and contains no password, note, title, tag, or key material. Locked metadata and unlocked vault responses use the same named-account preference rule: blank legacy records remain available only when no named exact-origin account exists. While the user types, the chooser remains only while the trimmed case-insensitive input is a prefix of a stored username; it closes on divergence. If no indexed username is available, non-empty typing closes the generic chooser. Every credential selection requires a fresh WebAuthn PRF ceremony when enrolled or a fresh master-password confirmation, even if a longer-lived manager root session is already unlocked. Showing an indexed account label never authorizes password release. The selected credential ID is bound into the authenticated extension-origin handoff. The master password is collected only in that protected window, never in content-script or page DOM. Both methods select only exact-HTTPS-origin logins, deliver one explicitly selected credential to the originating tab, and relock immediately. A prefilled username may differ in case but must otherwise match; its existing value is preserved. The inline chooser is fill-only and presents one row per usable account; it does not duplicate a single account with a separate submit action.
